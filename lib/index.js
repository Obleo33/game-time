...
function spawnSeal() {
  if (spawnCount >= 0) {
    ...
    if (seals.length< 5) {
      seals.push(new Seal({context: context,canvas: canvas,maxSpeed: 3,minSpeed:1}));
...
function stabDetect() {
  for (var i = 0; i < seals.length; i++) {
    ...
    if (narwhal.tuskX >= targetSeal.x && narwhal.tuskX <= (targetSeal.x + targetSeal.width) && narwhal.tuskY <= (targetSeal.y + targetSeal.height) && narwhal.tuskY >= (targetSeal.y + 28) && (narwhal.flipperKick || narwhal.burst)){
