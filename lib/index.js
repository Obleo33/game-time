...
function spawnSeal() {
  if (spawnCount >= 0) {
    ...
    if (seals.length< 5) {
      seals.push(new Seal({context: context,canvas: canvas}));
...
requestAnimationFrame(function gameStart(){
  clickToStart(canvas.width*.5,canvas.height*.5)
  //175,300
})
...
$('#game').on('click',function(){
  ...
  if(timer>0){
    //Ice
      ...
      context.clearRect(0, 0, canvas.width, canvas.height);
      gameOver(canvas.width*.5,200)
      drawScore(canvas.width*.5,250,"center")
      clickToStart(canvas.width*.5,400)
      $('#game').on('click', function(){
        timer = 2000
        score = 0
        requestAnimationFrame(gameLoop)
